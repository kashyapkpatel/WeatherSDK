...
open class BaseRepository @Inject constructor(open val context: Context) {
    ...
    private suspend fun <T> extractResultResource(call: suspend () -> Response<T>): Resource<T> {
        return try {
            ...
            if (response.isSuccessful) {
                ...
                val errorResponse = gson.get().fromJson(errBodyString, com.kashyapkpatel.weathersdk.network.models.ErrorResponse::class.java)
    ...
    private fun parseErrorResponse(throwable: Throwable?): com.kashyapkpatel.weathersdk.network.models.ErrorResponse? {
        var errorResponse: com.kashyapkpatel.weathersdk.network.models.ErrorResponse? = null
        throwable?.let {
            if (it is HttpException) {
                val responseBodyJson = it.response()?.errorBody()?.string()
                errorResponse =  gson.get().fromJson(responseBodyJson, com.kashyapkpatel.weathersdk.network.models.ErrorResponse::class.java)
            }
        }
        return errorResponse
    }

    private fun parseErrorMessage(errorResponse: com.kashyapkpatel.weathersdk.network.models.ErrorResponse?): String {
